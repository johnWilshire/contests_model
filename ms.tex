...
The probability of a male that covers $A_{searched}$ in one time step encountering any nest is:
\begin{equation}
    P(\rm {discovery}) = \frac {N \cdot 2r\cdot v} {A_{total}}
\end{equation}
Let $t_{mat}$ be the time when a male matures.
Now the maximum time possible time that a male can spend searching is $t_{max} = t_{females mature} - t_{mat}$

The expected number of events is $$ P( \rm {discovery}) \cdot t_{max}$$.
We then use this as the $\lambda$ parameter to draw the number of nest discovery events for this individual $n$ from the poisson distribution.

We then pull $n$ numbers from the uniform distrobution with lower bound $t_{mat}$ and upper bound $t_{females mature}$ as the times of these discovery events.

%I think I might use a priority queue for this... 
